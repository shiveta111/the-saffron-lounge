...
async function proxyToBackend(request: NextRequest, method: string, body?: any) {
  try {
    ...
    const backendUrl = `${API_BASE_URL}/api/v1/contact${url.search}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: 'Invalid request body', error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 400 }
    );
  }
}